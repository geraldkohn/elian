...
func agencySetRPermissionHandler(w http.ResponseWriter, r*http.Request) {
	token := r.Header.Get("Authorization")
	if r.Method == "POST" {
		var req agencySetReadOnlyPermissionReq
		var res agencySetReadOnlyPermissionRes
		defer func() {
		...
		rpcReq := &pb.AgencySetReadOnlyPermissionRequest{
			AgencyToken:         token,
			RecordUid:          req.RecordUid,
			PermissionRequests: permissionReqs,
		}
		rpcRes, err := c.AgencySetReadOnlyPermission(ctx, rpcReq)
		if err != nil {
